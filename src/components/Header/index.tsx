...
const Header = () => {
  ...
  return (
    <>
      <header
        className="header top-0 left-0 z-40 flex w-full items-center fixed z-[9999] bg-primary backdrop-blur-sm shadow-md transition-all"
      >
          ...
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-60 max-w-full px-4 xl:mr-12">
              ...
              >
                <div className="flex items-center">
                  <span className="text-2xl font-bold text-white">ST. JUDE'S</span>
                </div>
              </Link>
            </div>
            <div className="flex w-full items-center justify-between px-4">
              <div>
                ...
                <nav
                  id="navbarCollapse"
                  className={`navbar border-white/20 absolute right-0 z-30 w-[250px] rounded border-[.5px] bg-primary px-6 py-4 duration-300 lg:visible lg:static lg:w-auto lg:border-none lg:!bg-transparent lg:p-0 lg:opacity-100 ${navbarOpen
                        ...
                        {menuItem.path ? (
                          ...
                          <>
                            <p
                              ...
                              className="text-white/90 group-hover:text-white flex cursor-pointer items-center justify-between py-2 text-base lg:mr-0 lg:inline-flex lg:px-0 lg:py-6"
                            >
                              {menuItem.title}
                              <span className="pl-3">
                            ...
                            >
                              {menuItem.submenu.map((submenuItem, index) => (
                                <Link
                                  ...
                                  key={index}
                                  className="text-primary/70 hover:text-primary block rounded-sm py-2.5 text-sm lg:px-3"
                                >
                                  {submenuItem.title}
                                </Link>
                              ))}
